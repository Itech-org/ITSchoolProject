...
//  };
  ...
  if (document.querySelector(".main-header__button-change-speciality") != undefined) {
    ...
    document.querySelectorAll(".main-header__wrapp-for-speciality-link").forEach((elem) => elem.addEventListener("click", changeGroup));
    function changeGroup() {
      let value = this.getAttribute("data-value");
      document.querySelector(".main-header__change-course").value = value;
      console.log(value);
      document.querySelector(".main-header__button-change-speciality").innerHTML = this.innerHTML;
      this.parentNode.classList.add("no-active");
    }
  }

  // открыть(скрыть) статус

  if (document.querySelector(".main-header__button-change-status") != undefined) {
    document.querySelector(".main-header__button-change-status").onclick = function () {
      document.querySelector(".main-header__list-status").classList.toggle("no-active");
    };

    document.querySelector(".main-header__list-status").onmouseout = function () {
      document.querySelector(".main-header__list-status").classList.add("no-active");
    };

    document.querySelector(".main-header__list-status").onmouseover = function () {
      document.querySelector(".main-header__list-status").classList.remove("no-active");
    };

    document.querySelectorAll(".main-header__wrapp-for-status-link").forEach((elem) => elem.addEventListener("click", changeStatus));
    function changeStatus() {
      let value = this.getAttribute("data-value-status");
      console.log(value);
      document.querySelector(".main-header__change-status").value = value;
      document.querySelector(".main-header__button-change-status").innerHTML = this.innerHTML;
      this.parentNode.classList.add("no-active");
      console.log(document.querySelector(".students-home-work__date-change").value);

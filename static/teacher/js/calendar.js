  ...
  for (let task of tasks) {
    ...
    for (let card of cards) {
      if (card.getAttribute("data-date") == date[0]) {
        ...
        let link = cardTask;
        link.setAttribute("href", "http://127.0.0.1:8000/teacher/class/" + task.id.toString() + "/detail/");
      }
    }
  }
}
...
function createCard() {
  ...
  cardLink.setAttribute("href", "http://127.0.0.1:8000/teacher/class/");
...
async function request() {
      ...
      let response = await fetch("http://127.0.0.1:8000/teacher/api/v1/classes-list/" + strGET);
